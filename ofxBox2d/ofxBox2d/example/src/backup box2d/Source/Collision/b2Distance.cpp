...
// The origin is either in the region of BoxPoints[1] or in the edge region. The origin is
// not in region of BoxPoints[0] because that is the old BoxPoint.
static int32 ProcessTwo(b2Vec2* x1, b2Vec2* x2, b2Vec2* p1s, b2Vec2* p2s, b2Vec2* BoxPoints)
{
	// If in BoxPoint[1] region
	b2Vec2 r = -BoxPoints[1];
	b2Vec2 d = BoxPoints[0] - BoxPoints[1];
	float32 length = d.Normalize();
	float32 lambda = b2Dot(r, d);
	if (lambda <= 0.0f || length < B2_FLT_EPSILON)
	{
		// The simplex is reduced to a BoxPoint.
		*x1 = p1s[1];
		*x2 = p2s[1];
		p1s[0] = p1s[1];
		p2s[0] = p2s[1];
...
// Possible regions:
// - BoxPoints[2]
// - edge BoxPoints[0]-BoxPoints[2]
// - edge BoxPoints[1]-BoxPoints[2]
// - inside the triangle
static int32 ProcessThree(b2Vec2* x1, b2Vec2* x2, b2Vec2* p1s, b2Vec2* p2s, b2Vec2* BoxPoints)
{
	b2Vec2 a = BoxPoints[0];
	b2Vec2 b = BoxPoints[1];
	b2Vec2 c = BoxPoints[2];
...
static bool InBoxPoints(const b2Vec2& w, const b2Vec2* BoxPoints, int32 BoxPointCount)
{
	const float32 k_tolerance = 100.0f * B2_FLT_EPSILON;
	for (int32 i = 0; i < BoxPointCount; ++i)
	{
		b2Vec2 d = b2Abs(w - BoxPoints[i]);
		b2Vec2 m = b2Max(b2Abs(w), b2Abs(BoxPoints[i]));
				   ...
				   const T2* shape2, const b2XForm& xf2)
{
	...
	*x2 = shape2->GetFirstVertex(xf2);
	...
	for (int32 iter = 0; iter < maxIterations; ++iter)
	{
		...
		if (vSqr - vw <= 0.01f * vSqr || InBoxPoints(w, BoxPoints, BoxPointCount)) // or w in BoxPoints
		{
			if (BoxPointCount == 0)
			{
				*x1 = w1;
		...
		case 1:
			...
			p2s[1] = w2;
			BoxPoints[1] = w;
			BoxPointCount = ProcessTwo(x1, x2, p1s, p2s, BoxPoints);
			break;
		...
		case 2:
			...
			p2s[2] = w2;
			BoxPoints[2] = w;
			BoxPointCount = ProcessThree(x1, x2, p1s, p2s, BoxPoints);
			break;
		}

		// If we have three BoxPoints, then the origin is in the corresponding triangle.
		if (BoxPointCount == 3)
		{
			g_GJK_Iterations = iter;
			return 0.0f;
		...
		for (int32 i = 0; i < BoxPointCount; ++i)
		{
			maxSqr = b2Max(maxSqr, b2Dot(BoxPoints[i], BoxPoints[i]));
		}

#ifdef TARGET_FLOAT32_IS_FIXED
		if (BoxPointCount == 3 || vSqr <= 5.0*B2_FLT_EPSILON * maxSqr)
#else
		if (BoxPointCount == 3 || vSqr <= 100.0f * B2_FLT_EPSILON * maxSqr)
#endif
		{
			g_GJK_Iterations = iter;
			v = *x2 - *x1;
...
// GJK is more robust with polygon-vs-BoxPoint than polygon-vs-circle.
// So we convert polygon-vs-circle to polygon-vs-BoxPoint.
...
{
	BoxPoint BoxPoint;
	BoxPoint.p = b2Mul(xf2, circle->GetLocalPosition());

	float32 distance = DistanceGeneric(x1, x2, polygon, xf1, &BoxPoint, b2XForm_identity);
