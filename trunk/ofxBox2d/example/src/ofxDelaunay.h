...
public:
	...
	// ---------------------------------------------------------
	void draw(float w, float h, float sx, float sh) {
		for(int i=0; i<triangles.size(); i++) {
			...
			ofTriangle(ofMap(a.x, 0, w, 0.0, ofGetWidth()), 
					   ofMap(a.y, 0, h, 0.0, ofGetHeight()),
					   ofMap(b.x, 0, w, 0.0, ofGetWidth()),
					   ofMap(b.y, 0, h, 0.0, ofGetHeight()),
					   ofMap(c.x, 0, w, 0.0, ofGetWidth()),
					   ofMap(c.y, 0, h, 0.0, ofGetHeight()));
			ofNoFill();
			ofSetColor(10, 10, 10);
			ofTriangle(ofMap(a.x, 0, w, 0.0, ofGetWidth()), 
					   ofMap(a.y, 0, h, 0.0, ofGetHeight()),
